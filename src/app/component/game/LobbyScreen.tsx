...
import { SavedRunInfo } from "./saveSystem";
...
type Props = {
  ...
  addPlayer: () => void;
  startGame: () => void;
  savedRunInfo: SavedRunInfo | null;
...
export default function LobbyScreen({
  ...
  const savedAtLabel = savedRunInfo
    ? new Date(savedRunInfo.savedAt).toLocaleString("fr-FR", {
        dateStyle: "short",
        timeStyle: "short",
      })
    : null;
      ...
      <div className="relative z-10 w-full max-w-6xl px-6">
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-8 items-center">
          ...
          >
            ...
            {savedRunInfo && (
              <div className="mb-5 rounded-2xl border border-emerald-400/35 bg-emerald-950/25 p-4 shadow-[0_0_18px_rgba(16,185,129,0.16)]">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-emerald-50/80">
                      ...
                      {savedRunInfo.currentPlayerName
                        ? ` • Tour de ${savedRunInfo.currentPlayerName}`
                        : ""}
                    </div>
                    {savedAtLabel && (
                      <div className="mt-1 text-xs text-emerald-100/55">
                        Sauvegardée le {savedAtLabel}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="rounded-xl border border-emerald-300 bg-emerald-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-emerald-500"
                    >
                      Continuer
                    </button>
                    <button
                      type="button"
                      onClick={clearSavedRun}
                      className="rounded-xl border border-red-400/60 bg-red-950/40 px-4 py-2 text-sm font-bold text-red-100 transition hover:bg-red-900/70"
                    >
                      Effacer
                    </button>
                  </div>
