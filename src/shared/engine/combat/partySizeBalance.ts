...
function buildProfile(participantCount: number, enemies: Enemy[], isBossCombat: boolean): PartySizeBalanceProfile {
  ...
  if (safeCount <= 1) {
    return {
      ...
      hpMultiplier: hasBoss ? 0.86 : hasElite ? 0.78 : 0.74,
      offenseMultiplier: hasBoss ? 0.94 : hasElite ? 0.9 : 0.92,
      attackPowerMultiplier: hasBoss ? 1.08 : hasElite ? 1.12 : 1.16,
  ...
  if (safeCount === 2) {
    return {
      ...
      hpMultiplier: hasBoss ? 0.94 : 0.88,
      offenseMultiplier: hasBoss ? 0.96 : 0.92,
      attackPowerMultiplier: hasBoss ? 1.04 : 1.08,
