...
export function computeEnemyDamage(
  ...
  raw = Math.floor(raw * (0.9 + rng.next() * 0.25));
  const defenseReduction = Math.floor(playerData.defense / 4);
  const pressureFloor = Math.max(
    enemyData.isBoss ? 5 : 3,
    Math.floor(raw * (enemyData.isBoss ? 0.38 : 0.3)),
  );
...
export function computePlayerBasicAttackDamage(
  ...
  const reduced = Math.max(
    1,
    raw - Math.floor(enemyData.defense / 2),
  );
  const vulnerabilityMultiplier = getDebuffMultiplier(
    enemyStatuses,
    "vulnerability",
