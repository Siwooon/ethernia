...
const DEMO_TEAM: Array<{ name: string; classType: ClassType }> = [
...
type MobileLobbyPrototypeProps = {
...
export function MobileLobbyPrototype({
  entryMode = "solo",
  initialJoinCode = "",
  onBackHome,
  onOpenRun,
}: MobileLobbyPrototypeProps) {
  ...
  const applyDemoTeam = () => {
    const demoPlayers = DEMO_TEAM.map((hero, index) =>
      createPlayer({
        id: index + 1,
        name: hero.name,
    ...
    setRunSeed(createRunSeed());
    setFeedback("Groupe démo prêt. Tu peux ouvrir la faille.");
  };
  ...
  return (
    <EtherniaScreen>
      ...
      <EtherniaCard>
        ...
        <View style={styles.demoTeamBar}>
          <View style={styles.demoTeamTextBlock}>
            <Text style={styles.demoTeamTitle}>Rendu rapide</Text>
            <Text style={styles.demoTeamText}>Ajoute un groupe équilibré en un geste.</Text>
          </View>
          <EtherniaButton tone="secondary" onPress={applyDemoTeam}>
            Groupe démo
          </EtherniaButton>
        </View>
        ...
        <View style={styles.startSummary}>
          <Text style={styles.startLine}>Départ conseillé : 1 à 3 héros. Pour la démo, lance vite puis montre carte, combat et récompense.</Text>
