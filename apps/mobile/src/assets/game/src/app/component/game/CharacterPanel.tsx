...
import { getEquipmentBonuses } from "@/shared/lib/equipment";
...
export default function CharacterPanel({
  ...
  const renderInventoryActions = (item: InventoryItem) => {
    ...
    <AnimatePresence>
      {activeTab && (
        <motion.div
          initial={isMobile ? { y: 40, opacity: 0 } : { x: -30, opacity: 0, scale: 0.98 }}
          animate={isMobile ? { y: 0, opacity: 1 } : { x: 0, opacity: 1, scale: 1 }}
          exit={isMobile ? { y: 40, opacity: 0 } : { x: -30, opacity: 0, scale: 0.98 }}
          transition={{ duration: 0.2 }}
          className={isMobile
            ? "fixed inset-x-3 bottom-20 z-[65] max-h-[72vh] overflow-y-auto rounded-3xl border border-violet-700 bg-black/95 p-4 shadow-[0_-12px_35px_rgba(0,0,0,0.55)] backdrop-blur-md"
            : "absolute top-24 left-20 z-40 w-[360px] bg-black/90 border border-violet-700 rounded-xl p-4 shadow-[0_0_20px_rgba(168,117,255,0.25)] backdrop-blur-sm"
          }
        >
          {isMobile && (
            <div className="mb-3 flex items-center justify-between border-b border-violet-900/70 pb-3">
              <div className="text-xs uppercase tracking-[0.18em] text-violet-400">Panneau tactile</div>
              <button
                onClick={onClose}
                className="rounded-full border border-violet-500/70 bg-black/40 px-3 py-1 text-sm text-violet-100"
              >
                Fermer
              </button>
            </div>
