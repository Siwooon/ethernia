import { EventType, LocationTheme, MapNode, TerrainEffect } from "@/shared/types/game";
import { FloorBiome } from "@/shared/data/floors";
import { GameRandom, mathRandom, randomChance, randomPick, randomShuffle } from "@/shared/platform/random";
...
function rand<T>(arr: T[], rng: GameRandom): T {
  return randomPick(rng, arr);
}

function shuffle<T>(arr: T[], rng: GameRandom): T[] {
  return randomShuffle(rng, arr);
}
...
function toEventType(kind: GridKind, rng: GameRandom): EventType {
  switch (kind) {
    case "start":
      return "none";
    case "statuette":
    ...
    case "boss":
      return "boss";
    case "boss_prep":
      return randomChance(rng, 0.7) ? "rest" : "scripted_shrine";
    default:
      return "battle";
  }
}
...
function getDefaultTerrainEffects(
  ...
  if (biome === "cathedral" && randomChance(rng, 0.35)) {
    return [{ type: "mana_spring", value: 5, scope: "node" }];
  }

  if (randomChance(rng, 0.15)) {
...
function pickMany(
  ...
  return picked;
      ...
      function assignPathEvents(
          cells: TemplateCell[],
          bossPrepCell: TemplateCell | undefined,
          rng: GameRandom
        ...
        const randomCount = randomChance(rng, 0.5) ? 1 : 2;
...
const { distances, neighbors } = computeDistances(cells, startCell);
  ...
  const chosenTransform = rand(transformModes, rng);
  ...
  const pathAssignments = assignPathEvents(transformedCells, transformedBossPrepCell, rng);

  const nodes: MapNode[] = transformedCells.map((cell, index) => {
    const eventType =
      cell.kind === "path"
        ? pathAssignments.get(keyOf(cell.row, cell.col)) ?? "battle"
        : toEventType(cell.kind, rng);
    ...
    return {
      ...
      col: cell.col,
      kind: cell.kind,
      terrainEffects: getDefaultTerrainEffects(biome, eventType, rng),
      x: OFFSET_X + cell.col * CELL_SIZE,
      y: OFFSET_Y + cell.row * CELL_SIZE,
      lane: (Math.max(0, Math.min(2, Math.round((cell.row / Math.max(1, transformedMaxRow)) * 2))) as 0 | 1 | 2),
      depth: cell.col,
