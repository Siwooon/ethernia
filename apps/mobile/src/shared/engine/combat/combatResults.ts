...
export type CombatEndEvaluation = {
  ended: boolean;
  result: Extract<CombatResultKind, "victory" | "defeat"> | null;
};

export function buildCombatResultsFromAllies(
...
export function evaluateCombatEnd(params: {
  allies: CombatPlayerState[];
  enemies: CombatEnemyState[];
}): CombatEndEvaluation {
  const livingAllies = params.allies.filter(
    (ally) => !ally.isDead && ally.stats.hp > 0,
  );
  const livingEnemies = params.enemies.filter(
