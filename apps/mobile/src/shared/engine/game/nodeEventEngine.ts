...
function getCombatParticipantIndexes(state: EtherniaRunSave, nodeId: number): number[] {
  const participants = getAlivePlayerIndexesAtNode(state.players, nodeId);
  if (participants.length > 0) return participants;
  const active = state.players[state.currentPlayerIndex];
  return active && !active.isDead && active.stats.hp > 0 ? [state.currentPlayerIndex] : [];
}
...
export function buildMobileNodeEventPanel(state: EtherniaRunSave, nodeId: number): MobileNodeEventPanel | null {
  ...
  if (requiresNodeInteractionChoice(node)) {
    ...
    const participants = getCombatParticipantIndexes(state, node.id);
  ...
  return addClassChoiceToPanel(enrichMobileMapEventPanel(state, node, mapEventResultToPanel(node, eventResult, getCombatParticipantIndexes(state, node.id))), currentPlayer);
}
    ...
    case "take_statue": {
      ...
      return {
        ...
        panel: makeCombatPanel({
          ...
          participantIndexes: getCombatParticipantIndexes(state, node.id),
        }),
        logs: [],
      };
    }
...
export function resolveMobileImmediateEvent(
  ...
  let nextState = state;
  const panel = addClassChoiceToPanel(enrichMobileMapEventPanel(state, node, mapEventResultToPanel(node, result, getCombatParticipantIndexes(state, node.id))), player);
