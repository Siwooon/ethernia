...
export function canLocalDeviceControlPlayer(
  ...
  // A single device can own several local heroes in a multi-hero party.
  // The previous check only allowed the exact `localSeat`, which meant that
  // after using "Attendre" on the first hero, the next local hero could become
  // incorrectly locked behind "En attente de Place X".
  if (assignment.connection === "local") return true;
